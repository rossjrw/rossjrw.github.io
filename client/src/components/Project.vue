<template>
  ...
  >
    <div
      class="w-full h-72 relative z-0 -mt-56 -mb-16"
    ...
    >
      <div class="flex space-x-4">
        <div
          class="flex-initial flex-shrink-1 md:flex-shrink-0"
      ...
      <div
        class="basis-none flex justify-center flex-col"
      ...
      >
        ...
        >
          ...
          <div
            class="inline-links space-y-2 max-w-text"
            :class="[
    ...
    <div
      class="absolute bottom-0 right-0 m-2 opacity-80 text-sm font-display"
      :class="[
...
export default defineComponent({
  ...
  computed: {
    ...
    prettyDate(): string {
      let date = (this.project.date as number[][])
        .map((date) => date.join("–"))
        .join(", ")
    ...
    projectBackground(): string {
      if (
        this.project.size !== "big" &&
        !("back" in this.project) &&
        (this.project.size !== "small" ||
          !hasImage(this.project, "oblique"))
      ) {
        return "none"
      }
    ...
    attributionText(): string | null {
      ...
      return `<p>
        Image attribution: ${attributions
          .map((link, index) => {
            return `

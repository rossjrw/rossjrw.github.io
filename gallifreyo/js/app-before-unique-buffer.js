  ...
  function HostController($scope,$rootScope,$window,ShermanStorage){
    ...
    var renderWord = function(word,s_,w_,B_){
      ...
      for(let l = 0; l < word.letters.length; l++){
        ...
        var B = angles.slice(0,l).reduce((a, b) => a + b, 0) + l*buff; // this sums the first l angles
        renderLetter(word.letters[l],s_,w_,l,angles[l] - buff,radius,buff);
        
        word.letters[l].d = word.letters[l][0].path;
        word.letters[l].transform = "rotate("+B.toDeg()+","+0+","+radius+")";
      }
    };
    ...
    var renderLetter = function(letter,s_,w_,l_,angleSubtended,wordRadius,buff){
  ...
  function ShermanStorage(){
    ...
    return { // the following functions are available to controllers with ShermanStorage as dependency
      ...
      setAlphabet: function(betabet){
        ...
        alphabet = betabet; // still need to convert from JSON
      }
    };
  }
